...
base_metrics::define_metrics! {
    ...
    #[label(name = "outcome", default = ["inserted", "replaced"])]
    admitted: counter,
}
...
impl ValidityPoolMetrics {
    ...
    /// a fee bump or cancellation) from a net-new insertion. Together the two
    ...
    pub fn record_admission(replaced: bool) {
        Self::admitted(if replaced { "replaced" } else { "inserted" }).increment(1);
    }
}
...
mod tests {
    ...
    #[test]
    fn record_admission_splits_inserted_and_replaced() {
        ...
        assert_eq!(admitted_count(&snapshot, "inserted"), Some(2));
